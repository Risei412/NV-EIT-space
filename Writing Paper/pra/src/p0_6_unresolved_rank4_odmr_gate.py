...
For an equal-weight tetrahedral ensemble and any even single-line profile L,
...
With background-dominated white noise, orientation Fisher information scales as
B^8 in the unresolved low-field limit.
"""
...
# deterministic coherent-orientation strain stress test, 1.6--6.7 MHz
...
E_ZERO=np.zeros(4); PHI_ZERO=np.zeros(4)
...
def spectrum(Bglob,width=FWHM,ow=None,asym=0.,shift=0.,amp=CONTRAST,base=1.,
             Evals=E_STRESS,phis=PHI_STRESS):
...
def main():
    ...
    out=dict(run='RCI-20260810-P0-6',linewidth_MHz=30,contrast=CONTRAST,
        hyperfine_MHz=A14*1e3,strain_stress_GHz=E_STRESS.tolist(),rows=rows,
