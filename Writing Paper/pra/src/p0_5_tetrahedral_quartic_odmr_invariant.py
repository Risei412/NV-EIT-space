...
not require an orientation labeling at the level of the mathematical
observable.

Small-field result derived from the characteristic polynomial:

  lambda_0 = -p^2/D + (p^4 - p^2 q^2)/D^3 + O(B^6)

where q=gamma (n_a.B) and p^2=gamma^2(B^2-(n_a.B)^2).  Tetrahedral averaging
gives

  bar Omega_sum = 2D + 2 gamma^2/D B^2
                  - 2 gamma^4/D^3 (B^2)^2
                  + 4 gamma^4/(3D^3) (Bx^4+By^4+Bz^4) + O(B^6).

Thus rank 1 is absent, rank 2 is isotropic, odd rank is absent for this
field-even spectral sum, and rank 4 is the first direction-dependent term.
"""
...
def omega_sum_one(Bglob, n):
    """Sum of both ODMR transition frequencies for one NV orientation."""
    Bglob=np.asarray(Bglob,float); n=np.asarray(n,float)
    u=float(Bglob@n)
    bp2=max(float(Bglob@Bglob-u*u),0.0)
    # Spectrum depends only on B_parallel and |B_perp|; choose local B_y=0.
    H=D*(SZ@SZ)+GAMMA*(np.sqrt(bp2)*SX+u*SZ)
    w=np.linalg.eigvalsh(H)
    # In the declared small-field regime the m_s=0-connected eigenvalue is
    # uniquely the eigenvalue nearest zero.
...
def centroid8(Bglob):
    """Mean frequency of all 8 orientation x +/- ODMR transitions."""
    return 0.5*omega_bar(Bglob)


def analytic_omega_bar(Bglob):
    B=np.asarray(Bglob,float)
    b2=float(B@B); s4=float(np.sum(B**4))
    return float(2*D + 2*GAMMA**2/D*b2
                 -2*GAMMA**4/D**3*b2**2
                 +4*GAMMA**4/(3*D**3)*s4)


def direction(name,r):
    if name=='100': return np.array([r,0.,0.])
    if name=='110': return np.array([r,r,0.])/np.sqrt(2)
    if name=='111': return np.array([r,r,r])/np.sqrt(3)
...
def main():
    ...
    for r in (1e-4,5e-4,1e-3,2e-3,5e-3):
        b100=direction('100',r); b110=direction('110',r); b111=direction('111',r)
        o100=omega_bar(b100); o110=omega_bar(b110); o111=omega_bar(b111)
        rows.append(dict(
            B_T=r,
            omega100_GHz=o100,
            omega110_GHz=o110,
    ...
    out=dict(
        ...
        tetrahedral_moments=dict(M1=M1.tolist(),M2=M2.tolist(),
                                 M3_xyz=float(M3[0,1,2])),
        ...
        tensor_selection=dict(rank1='forbidden',rank2='isotropic only',rank3='forbidden in this field-even spectral sum',
                              rank4='first direction-dependent tensor'),
        rank4_coefficients=dict(S4_in_Omega_bar=coef_s4,
    ...
    path=PRA/'results'/'tables'/'p0_5_tetrahedral_quartic_odmr_invariant.json'
