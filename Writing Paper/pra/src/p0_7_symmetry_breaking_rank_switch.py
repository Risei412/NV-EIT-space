...
U100=np.array([1.,0,0])
U110=np.array([1.,1.,0])/np.sqrt(2)
F=np.linspace(D-.15,D+.15,601)
...
def main():
    ...
    out=dict(
      ...
      stress_pattern=dict(
        epsilon=eps.tolist(),weights=w.tolist(),epsilon_l2=epsnorm,E=E.tolist(),E_fro=Efnorm,
        norm_identity_ratio=(Efnorm**2)/(epsnorm**2),E_xy=Exy,crossover_B_mT=float(Bc*1e3),
        crossover_from_eps_bound_mT=float(Bc_bound*1e3),quartic_to_rank2_L2_ratio=ratios),
      numerical_validation=dict(
        linewidth_MHz=FWHM*1e3,hyperfine_MHz=A14*1e3,
        equal_weight_spectral_difference_exponent=float(np.polyfit(np.log(small),np.log(req),1)[0]),
        imbalanced_spectral_difference_exponent=float(np.polyfit(np.log(small),np.log(rim),1)[0]),benchmarks=rows))
    path=Path(__file__).resolve().parents[1]/"results"/"tables"/"p0_7_symmetry_breaking_rank_switch.json"
