...
\end{equation}
%
Eliminating $s$ without ever dividing by $G_{g}$ --- so that the ideal
...
(b) Magnitude of the contrast where it is positive (colour scale) with the
sign-reversed region overlaid. Fixed parameters: $B_{z}=\SI{5}{\milli\tesla}$,
...
Extracting the exponent is harder than it looks, and we report a partly
negative result. Three complications compound. The residual is not the value
at exactly $\Bperp=0$, which is structurally degenerate and does not join
smoothly onto the small-field plateau; it must be fitted. At the temperatures
where the island exists the response saturates early --- by
$\Bperp\approx\SI{0.06}{\tesla}$ at \SI{70}{\kelvin} the contrast has already
reached its plateau value --- so the interval between the residual plateau and
saturation spans less than a decade. Within that interval the local
logarithmic slope sweeps continuously from above $8$ down through $2$ to $0$
rather than resting on a plateau.

Fitting $C=C_{\mathrm{res}}+a\Bperp^{n}$ over the pre-saturation range gives
excellent fits ($R^{2}>0.997$) but exponents that depend on the choice of
upper cutoff. Only at \SI{70}{\kelvin} is the fit stable against that choice
(spread $0.06$ across cutoffs), and there $n=2.5$; at \SI{55}{\kelvin} and
\SI{85}{\kelvin} the spread is $1.1$--$1.6$ and the exponent is not
determined. We therefore do not claim to have verified the quadratic law in
the island. What the data do establish is that the opening is superlinear in
every well-conditioned fit, and that it is consistent with a quadratic leading
term: the archived room-temperature campaign, where the dynamic range between
plateau and saturation is largest, returns $n=2.03$--$2.18$. The honest
statement is that the exponent is measurable where the field range is wide and
is not measurable where the island lives.
...
\begin{figure}[t]
\includegraphics[width=\columnwidth]{fig5_bperp_scaling.pdf}
\caption{Opening of the Raman pathway with transverse field, full
Liouvillian. Solid curves are fits of $C=C_{\mathrm{res}}+a\Bperp^{n}$ over the
pre-saturation range; the dotted line is a slope-2 reference. The fitted
exponent is quoted only where it survives a change of upper cutoff
(\SI{70}{\kelvin}); at the other temperatures the interval between the
residual plateau and saturation is too narrow to determine it, and the legend
says so.}
\label{fig:bperp}
\end{figure}
...
measurements are the $\Bperp^{2}$ opening at fixed temperature, the collapse
of the contrast with temperature over several decades, and above all the
change of sign near \SI{103}{\kelvin}, which no Autler--Townes mechanism
produces and which we find to be resolvable in seconds. A measurement that
