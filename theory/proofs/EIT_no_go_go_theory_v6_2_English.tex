...
Fix the rotating-frame sign convention so that a detuning enters a complex
generator with a \emph{plus} sign, $A=\gamma_{31}+i\Delta_p$ and
$\gamma_g=\gamma_{21}+i\delta_2$. This is the convention used throughout the
manuscript and in the symbolic check
\texttt{calculations/symbolic/verify\_lambda\_reduction.py}; earlier drafts of
this note used the complex-conjugate convention, which gives the same physics
but the conjugate expressions, and the two must not be mixed when reading the
two documents side by side. With that fixed, the first-order block system is
...
. This is the standard three-level EIT susceptibility (Fleischhauer,
Imamoglu and Marangos, Rev.~Mod.~Phys.~77, 633 (2005)) in the
convention fixed above, and it is Eq.~(4) of the manuscript written over a
common denominator.
