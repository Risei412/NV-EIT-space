...
complement. Throughout, $\chi_{\mathrm{full}}=\chi_{0}\,d^{\dagger}x$ is the
linear probe susceptibility of the operative block: first order in the probe,
with the control retained to all orders through $\rho^{(0)}$ and through $A$,
$B$, $C$ and $G_{g}$. Here $d$ is the probe dipole vector and $\chi_{0}$
collects the number density, the dipole strength and $\varepsilon_{0}\hbar$.
Two qualifications are worth stating because they bound what a vanishing
$\chi_{\mathrm{full}}$ can mean. It is a \emph{local} susceptibility:
inhomogeneous classes are averaged only after it has been formed. And it is
the susceptibility of the operative manifold alone --- the phonon sideband,
the singlet and every transition outside that manifold carry their own
response and are not in $\chi_{\mathrm{full}}$ --- so $\chi_{\mathrm{full}}=0$
is a statement about the targeted transition, not about the crystal. The
contrast defined in Eq.~\eqref{eq:contrast} is a ratio of absorptions, so
$\chi_{0}$ cancels from every number reported below.

We define the \emph{pathway-cut} counterfactual
...
No adiabatic elimination is used in obtaining it. For $N_{e}$ resolved
branches with a diagonal optical generator $A=\mathrm{diag}(a_{j})$ the same
elimination returns the multi-excited-state form~\cite{O2011C04}, in which
each block becomes a coherent sum over branches,
%
\begin{equation}
K_{12}=\sum_{j}\frac{d_{1j}^{*}d_{2j}}{a_{j}},
\qquad
\Xi=S_{1}-\frac{\beta K_{12}K_{21}}{\gamma_{g}+\beta S_{2}},
\label{eq:multilevel}
\end{equation}
%
which is the case relevant here: the two $^{3}E$ branches contribute to
$K_{12}$ with a relative phase, and it is that phase which reverses in
Sec.~\ref{sec:phaseslip}. Setting
