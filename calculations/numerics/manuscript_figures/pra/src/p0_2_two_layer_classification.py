...
Outputs
-------
results/tables/p0_2_two_layer_phase_diagram.csv
results/tables/p0_2_two_layer_summary.json
results/raw/p0_2_two_layer_phase_diagram.npz
results/figures/fig_p0_2_two_layer_phase_diagram.{png,pdf}
"""
...
def load_legacy() -> dict[tuple[float, float], dict]:
    path = RES / "tables" / "p1_phase_diagram.csv"
...
def main(quick: bool = False, jobs: int = 2) -> dict:
    ...
    joint_grid = np.array([[JOINT_CLASSES.index(lookup[(float(T), float(B))]["joint_class"])
                            for B in Bs] for T in Ts], dtype=int)
    np.savez(RES / "raw" / "p0_2_two_layer_phase_diagram.npz",
