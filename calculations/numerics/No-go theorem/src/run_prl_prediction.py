...
def main():
    res=ROOT.parents[2]/'results'
    outdir=res/'certificates'; figdir=res/'figures'; tabdir=res/'tables'
    for _d in (outdir,figdir,tabdir,res/'raw'): _d.mkdir(parents=True,exist_ok=True)
    ...
    np.savez_compressed(res/'raw'/'branch_resolved_phase_map.npz',T_K=Tgrid,Bx_T=Bgrid,C=Cmap,dA=dAmap,
    ...
    np.savez_compressed(res/'raw'/'field_robustness_map_70K.npz',Bx_T=Bxs,Bz_T=Bzs,C=Fmap,branch=Fbranch)
    ...
    np.savez_compressed(res/'raw'/'polarization_tolerance_70K.npz',probe_angle_deg=deg,control_angle_deg=deg,C=Pmap,
    ...
    with (outdir/'nogo_gate_metadata_summary.json').open('w',encoding='utf-8') as f:json.dump(summary,f,indent=2)
