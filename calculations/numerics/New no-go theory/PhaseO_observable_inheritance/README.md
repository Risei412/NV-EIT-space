...
- `src/gate_a_observable.py` — model-independent library: `SectorSpec`,
  ...
  Reuses `calculations/numerics/New no-go theory/src/core.py` unchanged.
- `src/model_specs.py` — `SectorSpec` builders: a synthetic three-class family
  ...
  (`calculations/numerics/No-go theorem/src/nv_model.py`).
...
Outputs: `results/certificates/gates_summary_gateO.json`,
...
`2π` / sign conventions per `results/tables/convention_table.md`,
