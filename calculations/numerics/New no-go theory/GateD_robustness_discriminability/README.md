...
`calculations/numerics/No-go theorem/src/{nv_reduced_kernel, phonon_rates, group_iv_model, nv_model,
signal_chain}`, `calculations/numerics/New no-go theory/src/core.py`, and
`calculations/numerics/New no-go theory/src/phase_n_exact_core.py` (ν(q) fan; the theory-side PhaseN
...
Outputs: `results/certificates/gates_summary_gateD.json`, `gate_d_robustness.csv`,
