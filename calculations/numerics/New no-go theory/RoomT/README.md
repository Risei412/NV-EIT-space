...
Applies `calculations/numerics/No-go theorem/src/signal_chain.py` (built during this campaign's
...
Reuses `calculations/numerics/No-go theorem/src/gate2_candidate_full_vs_reduced.py`'s
...
1. **Wrong sign convention in the objective (the important one).** The
   ...
   this repository (`No-go theorem/`'s gates/active/SIMULATION_PLAN.md / PRL
...
adjugate/determinant method (`calculations/numerics/New no-go theory/src/core.py`'s Theorem 8.1
...
branches X/Y), reusing `calculations/numerics/No-go theorem/src/gate2_candidate_full_vs_reduced.py`'s
...
- `calculations/numerics/No-go theorem/src/nv_model.py` -- the physical NV branch-resolved
...
- `calculations/numerics/New no-go theory/src/model_lambda.py` -- the clean three-level Lambda
